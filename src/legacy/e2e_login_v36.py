...
async def run_e2e_v36():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 5. V36: Form Existence Loop with Refresh
            print("⏳ V36: Robust Wait for Form (Max 60s)...")
            element = None
            selectors = [
                'input[name="email"]',
                'input[type="email"]',
            ...
            for attempt in range(max_retries + 1):
                if attempt > 0:
                    print(f"   🔄 Attempt {attempt+1}: Page Refreshed. Waiting for form...")
                    await page.reload(wait_until="load", timeout=20000)
                    await page.wait_for_timeout(20000) # Extra wait after refresh
            ...
            if not element:
                ...
                with open("/tmp/vfs_v36_dump.html", "w", encoding="utf-8") as f:
                    f.write(html)
                await page.screenshot(path="screenshots/e2e_v36_debug.png")
                await browser.close()
                return
            ...
            if 'otp' in final_url.lower() or 'verify' in final_url.lower() or 'dashboard' in final_url.lower():
                ...
                await page.screenshot(path="screenshots/e2e_v36_success.png")
            else:
                print("⚠️ Beklenmeyen Yönlendirme.")
                await page.screenshot(path="screenshots/e2e_v36_result.png")

        except Exception as e:
            print(f"❌ Error: {e}")
            await page.screenshot(path="screenshots/e2e_v36_error.png")
        finally:
            await browser.close()

asyncio.run(run_e2e_v36())
