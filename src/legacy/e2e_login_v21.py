...
async def run_e2e_v21():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("⏳ Waiting for Cloudflare Challenge to resolve (Max 25s)...")
            try:
                await page.wait_for_selector("#cf-hcaptcha-container", state="detached", timeout=25000)
                print("✅ Cloudflare Challenge Solved!")
            except Exception:
                print("⚠️ Cloudflare challenge timeout. Proceeding anyway.")
            
            # Short buffer for Angular to render inputs after CF clears
            ...
            if 'otp' in current_url.lower():
                ...
                await page.screenshot(path="screenshots/e2e_v21_result.png")
            
        except Exception as e:
            print(f"❌ Playwright Error: {e}")
            await page.screenshot(path="screenshots/e2e_v21_error.png")
            print(f"🔗 Current URL: {page.url}")
        finally:
            await browser.close()

asyncio.run(run_e2e_v21())
