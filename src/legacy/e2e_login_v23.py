...
async def run_e2e_v23():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Dynamic wait for Cloudflare Challenge to resolve (iframe disappears)
            print("⏳ Waiting for Cloudflare Challenge to resolve (Max 15s)...")
            try:
                await page.wait_for_selector("#cf-hcaptcha-container", state="detached", timeout=15000)
                print("✅ Cloudflare Challenge Solved!")
            except Exception:
                print("⚠️ Cloudflare challenge timeout. Proceeding.")

            # Short buffer for Angular to render inputs
            print("⏳ Waiting 3s for Angular UI to hydrate...")
            ...
            if 'otp' in current_url.lower():
                ...
                print("⚠️ Not on OTP page. Checking for 404 redirect...")
                if 'page-not-found' in current_url.lower():
                    print("🔄 Redirected to 404, reloading to stabilize session...")
                    await page.reload(wait_until="domcontentloaded", timeout=20000)
                    await page.wait_for_timeout(3000)
                    print(f"🔗 Post-Reload URL: {page.url}")
                await page.screenshot(path="screenshots/e2e_v23_result.png")
            
        except Exception as e:
            print(f"❌ Playwright Error: {e}")
            await page.screenshot(path="screenshots/e2e_v23_error.png")
            print(f"🔗 Current URL: {page.url}")
        finally:
            await browser.close()

asyncio.run(run_e2e_v23())
