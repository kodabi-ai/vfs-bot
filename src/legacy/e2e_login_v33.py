...
async def run_e2e_v33():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("⏳ Waiting for Cloudflare Challenge to clear (iframe detach)...")
            try:
                await page.locator('iframe[src*="challenge"]').first.wait_for(state="detached", timeout=15000)
                print("✅ Cloudflare iframe detached.")
            except:
                print("⚠️ Cloudflare iframe wait timed out, continuing...")

            # 4. V33 CRITICAL: Wait extra 20s to ensure DOM fully updates after challenge clear
            print("⏳ CRITICAL WAIT: 20s for page content to stabilize...")
            ...
            if not element:
                ...
                with open("/tmp/vfs_v33_dump.html", "w", encoding="utf-8") as f:
                    f.write(html)
                await page.screenshot(path="screenshots/e2e_v33_debug.png")
                await browser.close()
                return
            ...
            if 'otp' in final_url.lower() or 'verify' in final_url.lower() or 'dashboard' in final_url.lower():
                ...
                await page.screenshot(path="screenshots/e2e_v33_result.png")

        except Exception as e:
            print(f"❌ Error: {e}")
            await page.screenshot(path="screenshots/e2e_v33_error.png")
        finally:
            await browser.close()

asyncio.run(run_e2e_v33())
