    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not element:
                ...
                with open("/tmp/vfs_v34_dump.html", "w", encoding="utf-8") as f:
                    f.write(html)
                await page.screenshot(path="screenshots/e2e_v34_debug.png")
                await browser.close()
                return
            ...
            if 'otp' in final_url.lower() or 'verify' in final_url.lower() or 'dashboard' in final_url.lower():
                ...
                await page.screenshot(path="screenshots/e2e_v34_success.png")
            else:
                print("⚠️ Beklenmeyen Yönlendirme.")
                await page.screenshot(path="screenshots/e2e_v34_result.png")

        except Exception as e:
            print(f"❌ Error: {e}")
            await page.screenshot(path="screenshots/e2e_v34_error.png")
        finally:
            await browser.close()

asyncio.run(run_e2e_v34())
