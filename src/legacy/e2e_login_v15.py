...
async def run_e2e_v15():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Wait for Cloudflare Challenge to resolve dynamically
            print("⏳ Waiting for Cloudflare Challenge to resolve...")
            try:
                await page.wait_for_selector("#cf-hcaptcha-container", state="detached", timeout=35000)
                print("✅ Cloudflare Challenge Solved!")
            except Exception:
                print("⚠️ Cloudflare challenge timeout or already gone. Proceeding anyway.")
            ...
            if 'otp' in current_url.lower():
                ...
                await page.screenshot(path="screenshots/e2e_v15_result.png")
            
        except Exception as e:
            print(f"❌ Playwright Error: {e}")
            await page.screenshot(path="screenshots/e2e_v15_error.png")
            print(f"🔗 Current URL: {page.url}")
        finally:
            await browser.close()

asyncio.run(run_e2e_v15())
