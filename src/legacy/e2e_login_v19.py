...
async def run_e2e_v19():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("⏳ Waiting for Cloudflare Turnstile to resolve (Max 30s)...")
            try:
                await page.wait_for_selector("#cf-turnstile", state="detached", timeout=30000)
                print("✅ Cloudflare Turnstile Solved!")
            except Exception:
                print("⚠️ Turnstile not found or timeout. Proceeding.")
            
            # Wait for Angular/React to render inputs
            print("⏳ Waiting 5s for UI to hydrate...")
            ...
            if 'otp' in current_url.lower():
                ...
                await page.screenshot(path="screenshots/e2e_v19_result.png")
            
        except Exception as e:
            print(f"❌ Playwright Error: {e}")
            await page.screenshot(path="screenshots/e2e_v19_error.png")
            print(f"🔗 Current URL: {page.url}")
        finally:
            await browser.close()

asyncio.run(run_e2e_v19())
