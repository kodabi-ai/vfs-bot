...
async def run_e2e_v22():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Dynamic wait for Cloudflare Challenge to resolve
            print("⏳ Waiting for Cloudflare Challenge to resolve (Max 30s)...")
            try:
                await page.wait_for_selector("#cf-hcaptcha-container", state="detached", timeout=30000)
                print("✅ Cloudflare Challenge Solved!")
            except Exception:
                print("⚠️ Cloudflare challenge timeout. Proceeding.")

            # Refresh to ensure cookies are fully applied and session is stable
            print("🔄 Refreshing page to stabilize session...")
            await page.reload(wait_until="domcontentloaded", timeout=30000)

            # Wait for Angular UI
            print("⏳ Waiting 10s for UI to hydrate...")
            ...
            if 'otp' in current_url.lower():
                ...
                await page.screenshot(path="screenshots/e2e_v22_result.png")
                await page.screenshot(path="screenshots/e2e_v22_full.png", full_page=True)
            
        except Exception as e:
            print(f"❌ Playwright Error: {e}")
            await page.screenshot(path="screenshots/e2e_v22_error.png")
            print(f"🔗 Current URL: {page.url}")
        finally:
            await browser.close()

asyncio.run(run_e2e_v22())
